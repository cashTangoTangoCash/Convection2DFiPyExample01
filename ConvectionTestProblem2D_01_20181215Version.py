#20181215: looking at https://github.com/guyer/Convection2DFiPyExample01/commit/ca59e8955319545f9965705c8fadfcbb5abd5951
...
if whichMesh=='second':
    cellSize=.025*(R_outer-R_inner)  #SETTING  must match what is in .geo file
    filename='infiniteCylinder01_firstRefinement.msh'
    meshed=True
    
if whichMesh=='third':
    cellSize=.0125*(R_outer-R_inner)  #SETTING  must match what is in .geo file
    filename='infiniteCylinder01_secondRefinement.msh'
    meshed=True

logging.info('using %s mesh' % whichMesh)
print 'using %s mesh' % whichMesh
assert meshed, 'mesh is not identified; please check this code'
...
dt=.9*dt_explicit
steps=int(tFinal/dt)
print 'total number of steps will be %d' % steps
val=dt/dt_explicit
...
for step in range(steps):
    ...
    if (step>0) and (step % plotEvery==0):
        ...
        if showAnalytical:
            ...
            logging.info('step is %d; min T along ray analytical solution is %.2E, max T along ray analytical solution is %.2E' % (step,min(T_a[0],T_a[-1]),max(T_a[0],T_a[-1])))
